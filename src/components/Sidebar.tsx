...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            ...
            <div className={`
                fixed inset-y-0 left-0 z-50 w-64 bg-[#161a1d] border-r border-[#2d343a] flex flex-col h-full shrink-0 transition-transform duration-300 ease-in-out md:relative md:translate-x-0
                ${isOpen ? 'translate-x-0' : '-translate-x-full'}
            `}>
                <div className="p-6 border-b border-[#2d343a] flex justify-between items-center">
                    {/* Branding: GRAVIS Logo Only */}
                    <div className="flex flex-col gap-1">
                        <Image
                            ...
                            priority
                        />
                        <p className="text-[8px] text-[#edf2f4]/30 uppercase tracking-[0.3em] ml-1">Design Intelligence</p>
                    </div>

                    {/* Mobile Close Button */}
                    <button
                        onClick={onClose}
                        className="md:hidden text-[#edf2f4]/50 hover:text-white p-1"
                    >
